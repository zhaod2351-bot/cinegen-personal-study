...
export function buildChatGptImportPrompt(
  rawStory = "",
  creativeContext = "",
): string {
  return `你是影视分镜导演。请将下面故事润色为可拍摄的短剧脚本，并且只输出一个合法 JSON 对象，不要 Markdown、解释或代码围栏。\n\nJSON 格式：\n{\n  "title":"项目标题",\n  "genre":"类型",\n  "logline":"一句话梗概",\n  "polishedScript":"润色后的完整剧本",\n  "targetDuration":"60s",\n  "language":"中文",\n  "characters":[{"name":"角色名","gender":"性别","age":"年龄","personality":"性格/剧情功能","visualPrompt":"可用于角色参考图的视觉描述"}],\n  "scenes":[{"name":"场景名","time":"时间","atmosphere":"氛围","visualPrompt":"可用于场景参考图的视觉描述"}],\n  "props":[{"name":"道具名","description":"用途与剧情功能","visualPrompt":"可用于道具参考图的视觉描述"}],\n  "storyParagraphs":[{"text":"按场次拆分的剧情段落","scene":"场景名"}],\n  "shots":[{"title":"镜头标题","scene":"场景名","actionSummary":"角色动作、表演与画面内容","dialogue":"对白或旁白，可为空字符串","cameraMovement":"运镜说明","shotSize":"景别，例如中景 MS","characters":["角色名"],"props":["道具名"]}]\n}\n\n严格要求：shots.scene 必须使用 scenes 中已有的 name；shots.characters 必须使用 characters 中已有的 name；shots.props 必须使用 props 中已有的 name，没有道具时使用空数组；每个镜头都要有动作和运镜。\n\n待处理故事：\n${rawStory || "（请在此粘贴你的故事或剧本）"}`;
}
...
export function parseImportedStoryPlan(json: string): ImportedStoryPlan {
  ...
  try {
    ...
    throw new Error(
      "无法读取 JSON。请只粘贴 ChatGPT 输出的完整 JSON，不要包含 Markdown 标记。",
    );
  }
  ...
  const rawScript = text(
    pick(root, "polishedScript", "script", "剧本", "润色剧本"),
    "polishedScript",
  );
  if (!rawScript) throw new Error("polishedScript 必须是非空文本");

  const rawCharacters = list(
    pick(root, "characters", "人物", "角色"),
    "characters",
  );
  const characters: Character[] = rawCharacters.map((value, index) => {
    ...
    return {
      ...
      personality: text(
        pick(item, "personality", "性格"),
        `characters[${index}].personality`,
        "未设定",
      ),
      visualPrompt: text(
        pick(item, "visualPrompt", "视觉描述", "提示词"),
        `characters[${index}].visualPrompt`,
      ),
      variations: [],
    };
  });
  ...
  const scenes: Scene[] = rawScenes.map((value, index) => {
    ...
    return {
      ...
      atmosphere: text(
        pick(item, "atmosphere", "氛围"),
        `scenes[${index}].atmosphere`,
        "未设定",
      ),
      visualPrompt: text(
        pick(item, "visualPrompt", "视觉描述", "提示词"),
  ...
  const sceneByName = new Map(
    scenes.map((scene) => [scene.location, scene.id]),
  );
  const rawPropsValue = pick(root, "props", "道具");
  const props: PropAsset[] = (
    Array.isArray(rawPropsValue) ? rawPropsValue : []
  ).map((value, index) => {
    const item = asRecord(value, `props[${index}]`);
    const name = text(pick(item, "name", "名称"), `props[${index}].name`);
    if (!name) throw new Error(`props[${index}].name 不能为空`);
    return {
      id: `import-prop-${index + 1}`,
      name,
      description: text(
        pick(item, "description", "描述", "用途"),
        `props[${index}].description`,
        "未设置",
      ),
      visualPrompt: text(
        pick(item, "visualPrompt", "视觉描述", "提示词"),
        `props[${index}].visualPrompt`,
      ),
    };
  });
  const propByName = new Map(props.map((prop) => [prop.name, prop.id]));
  ...
  const shots: Shot[] = rawShots.map((value, index) => {
    ...
    if (!sceneId)
      throw new Error(
        `shots[${index}].scene “${sceneName}” 未在 scenes 中定义`,
      );
    const names = list(
      pick(item, "characters", "角色"),
      `shots[${index}].characters`,
    ).map((name, characterIndex) =>
      text(name, `shots[${index}].characters[${characterIndex}]`),
    );
    const characterIds = names.map((name) => {
      const id = characterByName.get(name);
      if (!id)
        throw new Error(
          `shots[${index}].characters 中的 “${name}” 未在 characters 中定义`,
        );
      return id;
    });
    const propNamesValue = pick(item, "props", "道具");
    const propIds = (Array.isArray(propNamesValue) ? propNamesValue : []).map(
      (name, propIndex) => {
        const propName = text(name, `shots[${index}].props[${propIndex}]`);
        const id = propByName.get(propName);
        if (!id)
          throw new Error(
            `shots[${index}].props 中的“${propName}”未在 props 中定义`,
          );
        return id;
      },
    );
    return {
      ...
      sceneId,
      actionSummary: text(
        pick(item, "actionSummary", "action", "动作"),
        `shots[${index}].actionSummary`,
      ),
      dialogue: text(
        pick(item, "dialogue", "对白"),
        `shots[${index}].dialogue`,
        "",
      ),
      cameraMovement: text(
        pick(item, "cameraMovement", "camera", "运镜"),
        `shots[${index}].cameraMovement`,
      ),
      shotSize: text(
        pick(item, "shotSize", "size", "景别"),
        `shots[${index}].shotSize`,
        "中景 MS",
      ),
      ...
      keyframes: [
        {
          id: `import-shot-${index + 1}-start`,
          type: "start",
          visualPrompt: "",
          status: "pending",
        },
        {
          id: `import-shot-${index + 1}-end`,
          type: "end",
          visualPrompt: "",
          status: "pending",
        },
  ...
  const storyParagraphs = Array.isArray(rawParagraphs)
    ? rawParagraphs.map((value, index) => {
        ...
        const sceneName = text(
          pick(item, "scene", "场景"),
          `storyParagraphs[${index}].scene`,
        );
        const sceneRefId = sceneByName.get(sceneName);
        if (!sceneRefId)
          throw new Error(
            `storyParagraphs[${index}].scene “${sceneName}” 未在 scenes 中定义`,
          );
        return {
          id: index + 1,
          text: text(
            pick(item, "text", "内容"),
            `storyParagraphs[${index}].text`,
          ),
          sceneRefId,
        };
      })
...
export function applyImportedStory(
  project: ProjectState,
  plan: ImportedStoryPlan,
